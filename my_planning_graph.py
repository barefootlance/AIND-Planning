        ...
        for s_level in self.s_levels:
            for pgNode_s in s_level:
                for goal in self.problem.goal:
                    if goal in foundGoals:
                        continue
                    if goal == pgNode_s.symbol and pgNode_s.is_pos: # TODO: can goal be negative?
                        foundGoals.add(goal)
                        level_sum += level
            level += 1
        return level_sum
